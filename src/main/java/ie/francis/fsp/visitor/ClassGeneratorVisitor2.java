...
public class ClassGeneratorVisitor2 implements Visitor {
  ...
  public ClassGeneratorVisitor2(String className, List<String> parameters, Environment environment) {
  ...
  private void compileMacroCall(Symbol symbol, Cons cons) {
    ...
    try {
      Object output = environment.loadClass(owner).getMethod("run", paramTypes).invoke(null, params.toArray());
...
//        compilePrognSpecialForm(cons);
  ...
  private void compileMacroSpecialForm(Cons cons) {
    ...
    String descriptor =
            "(" + "Ljava/lang/Object;".repeat(paramCount) + ")Ljava/lang/Object;";

    environment.put(macroName, new Macro(className + ".run", descriptor));
    ClassGeneratorVisitor2 cgv = new ClassGeneratorVisitor2(className, args, environment);
